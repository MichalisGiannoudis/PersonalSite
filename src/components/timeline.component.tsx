...
export const TimeLine = () => {
    ...
    return (
        <>
            ...
            <div className="p-8 relative min-h-[800px] overflow-hidden">            
                {visualize && (
                ...
                {!visualize && (
                    <div className="relative h-full">
                        {['idle', 'fading', 'shrinking'].includes(animationStep) && (
                            <div className={`absolute left-1/2 transform -translate-x-1/2 w-1 bg-gradient-to-b from-blue-500 to-purple-500 rounded-full h-full ${animationStep === 'shrinking' ? 'animate-shrink-line' : ''}`}></div>
                        )}
                        {['idle', 'fading'].includes(animationStep) && (
                            <div className={`space-y-12 ${animationStep === 'fading' ? 'animate-fade-out-box' : ''}`}>
                                {episodes.map((episode, index) => (
                                    <div key={episode.company} className="relative flex items-center">
                                        {index % 2 === 0 ? (
                                            <>
